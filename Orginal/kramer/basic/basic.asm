...
; V.Pohlers 05.08.2019 Anpassung an AS-Assembler, Fehlerkorrektur
; Listing kontrolliert anhand der Pruefsummen aus mc 05/1982, S.71
; 33 Fehler behoben
; fehlerfrei assemblierbar mit Arnold-Assembler
; (http://john.ccac.rwth-aachen.de:8000/as/)

;15.08.2019 Modifikation ala KRAMERMC-Basic
...
TOKEN_TIC       equ   0D5h
;
TOKEN_CONT      equ   0EBh		; last used token
...
; Jump table, modify these to point to own routines
                ...
                org 8000h
COLDSTART0:     jp    coldstart
WARMSTART0:     jp    recovered_msg
math_usr:       jp    0ff9h
...
loc_674:        ex    de, hl           ;     | restore target buf in DE
                ...
                cp    TOKEN_TIC-CHAR_COLON ;  | is it "tic"? (D5)
                ld    b, CHAR_COLON    ;     | put colon in buf
                jr    z, loc_694       ;     | yes, was D5
                cp    TOKEN_DATA-CHAR_COLON ; | is token 83 (DATA)?
...
ctrl_7f:        ld    a, b             ; get current char count of input line
                dec   a                ; decrement
                or    (iy+ioparams_curpos) ; get current position
...
write_char:     push  bc               ; print_char
                ...
                ld    a, (iy+ioparams_linelength) ; get line length
...
loop795:        call  outputvector     ;<----+ emit it
                inc   (iy+ioparams_curpos) ; | advance next position
                djnz  loop795          ;-----+ loop
...
loc_7A6:        cp    CHAR_SPACE       ; is it a CTRL?
                ...
                ld    a, (iy+ioparams_curpos) ; get current pos
                cp    (iy+ioparams_linelength) ; compare with line length
                call  z, print_crlf    ; if end of line reached, do CRLF
                inc   (iy+ioparams_curpos) ; advance position
...
loc_88B:        EXPECT TOKEN_TO        ; expect a TO
                ...
                PUSH_FPREG1            ; FORSTRUCT: push 6 bytes end value
                ...
                pop   hl               ; restore curlineptr
loc_8BD:        PUSH_FPREG1             ; FORSTRUCT: push 6 bytes of STEPsize
...
execute_token:  sub   80h              ; has a token or a single ASCII
                ...
                cp    TOKEN_CONT+1-TOKEN_ELSE  ; token larger than E9 are syntax errors
                jp    nc, syntax_error
...
loc_93C:        rlca                   ; token * 2 (word index)
                ...
                LDBC_M                 ; get command address into BC
...
skipspace:      ld    a, (hl)          ; get char from    buf
                cp    TOKEN_TIC        ; is it "tic" (token D5)?
...
fpaccu_to_16:   ld    a, (fpaccu_exp)  ; load exponent
                ...
                FPREG_CONST 9180h, 0, 0 ; constant 65536
...
do_assignment1: push  de               ; HL = curlineptr
                                       ; DE save var address
...
exec_print:     jp    z, print_crlf    ; end of statement, i.e. empty PRINT?
                ...
                ld    a, (iy+ioparams_curpos) ; get current cursor position
                add   a, (hl)          ; addstring length
                cp    (iy+ioparams_linelength) ; is string longer than rest of line?
...
printcomma:     ld    a, (iy+ioparams_curpos) ; get current print position
                cp    (iy+ioparams_last_field) ; is beyond last field position?
...
print_crlf:     ld    a, CHAR_CR       ; load CR
                ld    (iy+ioparams_curpos), a  ; set char position to anything
                call  write_char       ; emit byte
                ld    a, CHAR_LF       ; load LF
                call  write_char       ; emit

print_nul_delay:
                ld    a, (iy+ioparams_padcount) ; get NUL byte cntr
                inc   a                ; +1
loopCFF:        dec   a                ;<-------+ predecrement
                ld    (iy+ioparams_curpos), a ; |  clear charpos (A is 0 on exit)
                ret   z                ;        | exit if finished
                push  af               ;        | save cntr
                ld    a, (iy+ioparams_padchar) ;| get NUL byte
                call  write_char       ;        | emit
                pop   af               ;        | restore cntr
                jr    loopCFF          ;--------+ loop
...
loc_E02:        ex    (sp), hl         ; stack is data_ptr
                ...
                or	(hl)		;char from input line to set flags
...
expr_function:  ld    b, 0             ; high value
                ...
                call  nextchar         ; get next char
                ld    a, c
                cp    2*(TOKEN_CHRS-TOKEN_SGN)+1 ; less than CHR$()?
...
fpaccu_clear:   ld    (fpaccu_exp), a  ; A is 0
                ld    hl, memend       ; should be irrelevant, as exp=0
...
; process LPOS()
math_lpos:      ld    a, (prtparam+ioparams_curpos) ; get printer position
                jr    uA_to_fpaccu

; process POS()
math_pos:       ld    a, (conparam+ioparams_curpos) ; load current cursor position
...
fn_program:     or    a                ; | is a real EOLN?
                jr    nz, loop1260     ; | no, something follows
...
loc_137D:       ld    de, (fpaccu_mant32) ; | |    
                call  peekpop_str_stringstk ; |    	peekpop_str_exprstk
                ld    hl, straccu      ;  | |    
                call  move_to_var      ;  | |    
                jr    loop134F         ;--|-+
...
math_chrs:      ld    a, 1             ; set request size
                ...
                ld    hl, (straccu_addr) ; load address of reserved string
...
;;;                call  expression_u8_ae ; get exxor expression in E
		call	0841Bh	; vp:definitiv ein Fehler !!!
...
exec_width:     call  expression_u8_ae ; get an 8 bit expression
                ...
                ld    (iy+ioparams_linelength), a ; store as line width
...
exec_save:      call  check_alpha      ; get program name character
       		...
       		ld    bc, 8FFh         ;<+ load char FF and cntr 8
loop17D7:       call  PUNCHOUT         ; | punch tape
                djnz  loop17D7         ;-+ loop to emit 8 * FF
...
loop1988:       call  nextchar         ;<--+ get char
                ...
                push  hl               ;   | save curlineptr
                or    a                ;   | clear CY
                ld    hl, 38552        ;   | maximum lineno before adding last digit
...
;;                dec   (hl)             ; | | 
		jp	loc_ADF8
loc_9861:       jr    nz, loop1B54     ;-+ | no loop
...
loop1CA3:       call  z, print_crlf       ;<+ print a CRLF
...
loop1D41:       ld    a, (hl)          ;<+ get current char
                ...
                dec   a
                jp    z, loop4A9       ; char is zero?, clear line
amerge_enter_line:            
...
loop1F35:       push  de               ;<+ push call address on stack
                call  skipspace        ; | advance
                jr    z, call2b        ; | end of statement? done with arguments
                EXPECT CHAR_COMMA      ; | expect a comma
...
call2b:         ex    de, hl           ; DE is curlineptr
...
loop1FF6:       ld    b, a
       		ld    a, c             ; get highest mantissa
                or    a
      		jr    nz, loc_2022     ;<+ not zero, shift single
		ld    c, ixh            ; | high mantissa is 0, shift mantissa a byte
                                       ; | ixh -> C
                ld    a, ixl            ; | ixl -> ixh
                ld    ixh, a            ; | 
                ld    ixl, d            ; | D -> ixl
                ...
                sub   8                ; | subtract 8
                cp    0D0h             ; | 
                jr    nz, loop1FF6     ;-+ not yet zero? loop
...
loc_2034:       ld    hl, fpaccu_exp   ; get shift
                or	a
                call  m, mant_inc_r    ; if negative, round FPreg up
...
mant_inc_r:     inc   e                ; increment mantissa 2
                ret   nz               ; exit unless carry
                inc   d                ; increment mantissa 3
                ret   nz
                inc   ixl               ; increment mantissa 4
                ret   nz
                inc   ixh
                ret   nz               ; exit unless carry
...
fpreg_complement:
                ...
                ex    de, hl
                ld    l, a             ; L = 0
                sbc   a, ixl            ; complement ixl
                ld    ixl, a
                ld    a, l             ; A = 0
                sbc   a, ixh            ; complement ixh
                ld    ixh, a
                ld    a, l             ; A = 0
...
mant_shiftright1:            
                ...
                ld    a, ixh            ; shift right ixh
                rra
                ld    ixh, a            ; A -> ixh
loc_20B3:       ld    a, ixl            ; shift right ixl
                rra
                ld    ixl, a
                rr    d                ; shift right D
                rr    e                ; shift right E
...
LOG_poly_tbl:   db    6
                ...
                db    0CBh, 5Ch, 60h, 0BBh, 13h, 80h ; 0.57634547
...
math_log:       call  fpaccu_sgn       ; get sign
                ...
                FPREG_CONST 8031h,7217h,0F7D2h ; constant LOG(2)
...
loc_2168:       ld    a, (hl)          ; get factor byte
                ...
                ld    b, e             ; shift 8 bits
                                       ; mant2 -> mant1
                ld    e, d             ; mant3 -> mant2
                ld    d, ixl            ; mant4 -> mant3
                ex    af, af'
                ld    a, ixh            ; mant5 -> mant4
                ld    ixl, a
                ex    af, af'
                ld    ixh, c            ; mant6 -> mant5
                ld    c, a             ; 0 -> mant6
                ret
...
loc_2196:       rra                    ; | shift right mant6
                ld    c, a             ; | 
                ld    a, ixh            ; | shift right mant5
                rra                    ; | 
                ld    ixh, a            ; | 
                ld    a, ixl            ; | shift right mant4
                rra                    ; | 
                ld    ixl, a            ; | 
...
loc_239B:       ld    a, 0A8h          ; exponent for overflow
                ...
                rla                    ; move into CY
                call  c, mant_inc_r    ; increment for 2s complement
                ld    b, 0             ; mant1
...
do_period:      inc   c                ; | was there already a decimal point?
                jr    z, loop23F5      ;>+ no, loop to read more digits
; has complete number                  ; |
...
loc_2600:       push  hl               ; save HL
                ...
                jr    z, loop2616      ; do not replace
                ld    a, b             ; put char in A
                cp    c                ; is space?
                ld    c, CHAR_STAR     ; preload '*'
                jr    nz, loop2616     ; was not space, skip
...
loop2658:       push  af               ;<+ | save current character
                ...
                jr    nz, loop267C     ;   | no, non-zero digits start here
; discard leading zeroes to fit number into buf
                inc   hl               ;   | advance to next
                call  nextchar         ;   | get next char
                jr    nc, loop267C     ;   | not digit, skip
...
SIN_poly_tbl:   db    7
                ...
                db    0B1h, 4Fh, 7Fh, 3Bh, 28h, 86h    ; 42.05517315
                db    31h, 0B6h, 64h, 69h, 99h, 87h     ; -76.70584506
...
a_invalid_input:db    '*', 'I', 'N', 'V', 'A', 'L', 'I', 'D', ' ', 'I', 'N'
                db    'P', 'U', 'T', CHAR_LF+80h
a_at_line:      db    ' ', '@', ' ', 'L', 'I', 'N', 'E', 20h+80h
a_ready:        db    CHAR_LF, 'R', 'E', 'A', 'D', 'Y', ':', CHAR_LF+80h
a_extralost:    db    '*', 'E', 'X', 'T', 'R', 'A', ' ', 'L', 'O', 'S', 'T', CHAR_LF+80h
...
loc_2FCE:       ld    (hl), a          ; clear cell
                ...
                ld    a, 63            ; set line length to 72
                ld    (conparam+ioparams_linelength), a ; store in console parameter set
                ld    (prtparam+ioparams_linelength), a ; store in printer parameter set
                ld    a, 56            ; position of last complete print field
                                       ; print length is 14
                                       ; i.e. positions are 0,14,28,42,56
                ld    (conparam+ioparams_last_field), a ; store in console parameter set
                ld    (prtparam+ioparams_last_field), a ; store in printer parameter set
                ld    hl, stringstk    ; HL is addr of exprstack
                ld    (stringstkptr), hl ; store ptr
                ld    hl, 8000h            ; clear HL
                ld    (rnd_mant23), hl
                ld    (rnd_mant45), hl
                ld    (rndmant6_exp), hl
...
loc_306A:       jp    c, out_of_memory_error ; too few memory
                ...
                CPHL_DE   
                ld    de, 1001h ; start of disposable coldstart area
