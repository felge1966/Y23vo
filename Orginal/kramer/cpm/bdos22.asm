...
;character in accum, save it
		LD	(KBCHAR),A
CONB1:
;return with TRUE_ set in accumulator
		LD	A,1
		RET
;
CONOUT:
...
;look while arecord >= (t:=currec + sectpt)
		...
		JP	C,SEEK2	;can be > FFFFH         ;JP	C,SEEK2
		LD	A,c                             ;LD	A,ARECL
		SUB	l                               ;SUB	TCRECL
		LD	A,b                             ;LD	A,ARECH
		SBC	A,h                             ;SBC	A,TCRECH
		JP	C,SEEK2	;skip if t > arecord    ;JP	C,SEEK2
;currec = t
		EX	DE,HL                           ;EX	DE,HL
;curtrk = curtrk + 1
		POP	hl                              ;POP	CTRKH
		INC	hl                              ;INC	CTRKH
		JP	SEEK1	;for another try        ;JP	SEEK1
...
;searchl
		...
		CALL	END_OF_DIR
		JP	Z,SEARCH_FIN	;skip to end if so
